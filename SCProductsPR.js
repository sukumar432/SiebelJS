    ...
    function ()
    {
      SiebelAppFacade.SCProductsPR = (function ()
      {
        ...
        var RootOrderItemId = "",
          ...
          LineItemId = "",
          ControlBoxSku, ClickedShowOkbtn;
        ...
        SCProductsPR.prototype.BindData = function (bRefresh)
        {
          ...
          if (localStorage.getItem('flowfrom') != "proceed" && localStorage.getItem('addcart') != "addcart")
          { //it should neither run after proceed is clicked nor afer addtocart is clicked
            pm = this.GetPM();
            ...
            setTimeout(function ()
            {
              ...
              if (recordSet.length != 0)
              {
                if (TotalCount[4] != undefined)
                {
                  ...
                  if (isLast == 0 && recordSet.length <= 9)
                  { //if applet has less than 10 records
                    isLast = 1;
                  ...
                  else if (isLast == 0 && !(TotalCount[4].indexOf("+") > 0))
                  { //if don't contain + symbol it is last recordset and will append  for on last time
                    isLast = 1;
                    prodList = showProducts(recordSet, 10);
                    $("#SC_Products").append(prodList);
                    hideDuplicates();
                  }
                  else if (isLast == 0 && localStorage.getItem("Records") < parseInt(TotalCount[2]))
                  { //not the old recordset that we already showed
                    prodList = showProducts(recordSet, 10);
                    $("#SC_Products").append(prodList);
                    hideDuplicates();
                    localStorage.setItem("Records", parseInt(TotalCount[2]));
                  }
                  else if (isLast == 0 && ((TotalCount[4].replace("+", "")) < 9))
                  { //for dup check appends //mostly need to remove as issu got resolved
              ...
              else
              {
                ...
                if (localStorage.getItem("addToOrder") == "Y")
                {
                  ...
                  $('#custommaskoverlay').hide();
                  $('.modal-backdrop').remove(); //LikhithaK:09Sep2025:Added as part of Greout issue on Add to Order button click.
                  localStorage.setItem("addToOrder", "N");
                }
              }
        ...
        SCProductsPR.prototype.BindEvents = function ()
        {
          ...
          $(document).unbind(".deletelineitem").on("click", ".deletelineitem", function ()
          {
            ...
            setTimeout(function ()
            {
              ...
              if ($('#Ship-Option-Markup').is(':visible'))
              { //if shipping is visible then only do execute list applet frame
              ...
              updateShipping(""); //15Oct24;SL;Added for Tier Delivery;
          ...
          $(document).unbind(".cancellineitem").on("click", ".cancellineitem", function ()
          {
            ...
            //}, 1000);
            if ($('#Ship-Option-Markup').is(':visible'))
            { //if shipping is visibel then only do excute list applet frame
              localStorage.setItem('ProceedToShipping', 'Y');
              FieldQueryPair = {
                "SKU #": ""
              };
            ...
            setTimeout(function ()
            {
              ...
              else
              {
                if ((sdisamount >= 0) || (sdisper >= 0))
                {
                  ...
                  if (scmanualadminflag == "Y" && LOVValue == "")
                  { //STRY0207171 Updated condition
          ...
          $(document).on('click', '#sc-proceed', function ()
          {
			...
			//VYADAVAL-15May2025-Added for Protection plan by Extend
			$("[id$='_sc-line-desc']").each(function () {
				//console.log("Processing element:",$(this).attr("id"));
				var LineVal = $(this).val();
				var elemId1 = $(this).attr("id").split("_")[0];
				var inPS = SiebelApp.S_App.NewPropertySet();
				var outPS = SiebelApp.S_App.NewPropertySet();
				var Bservice;
				var fieldnames = "SC Warranty Ref Line Detail";
				var fieldvalues = LineVal;
				inPS.SetProperty("BO", "Order Entry (Sales)");
				inPS.SetProperty("BC", "Order Entry - Line Items");
            ...
            setTimeout(function ()
            {
              ...
              if (fitFlag == "Y" && poleDisplayFlg == "N" && resumeOrderflag != "Y")
			  {
				 //AKSHAY: commented below line for Terminal CR.
        ...
        function updateShipping(value) //15Oct24;SL;Added for tier delivery
        {
          var linItemRecordSet, numOfRec = 0, recIndex;
          var strShipMethod, strProductLn, strProductTypeCd;
          linItemRecordSet = SiebelApp.S_App.GetActiveView().GetAppletMap()["SC Order Entry Line Item List Applet"].GetBusComp().GetRecordSet();
          numOfRec = linItemRecordSet.length;
          if (numOfRec >= 0)
          {
            recIndex = numOfRec + 1;
            $("#" + recIndex + "_s_" + appletseqorderentry + "_l_SC_Product").trigger("click");
            strShipMethod = SiebelApp.S_App.GetActiveView().GetAppletMap()["SC Order Entry Line Item List Applet"].GetBusComp().GetFieldValue("Ship Method");
            strProductLn = SiebelApp.S_App.GetActiveView().GetAppletMap()["SC Order Entry Line Item List Applet"].GetBusComp().GetFieldValue("Product Line");
            strProductTypeCd = SiebelApp.S_App.GetActiveView().GetAppletMap()["SC Order Entry Line Item List Applet"].GetBusComp().GetFieldValue("Product Type Code");
          }
          if (strShipMethod == "Home Delivery" && strProductLn != 'DELIVERY' && strProductLn != 'SERVICE' && strProductTypeCd != 'Promotion')
          {
            for (var i = 0; i < numOfRec; i++)
            {
              if (linItemRecordSet[i]['Ship Method'] == "Home Delivery" && linItemRecordSet[i]['Product Line'] != 'DELIVERY' && linItemRecordSet[i]['Product Line'] != 'SERVICE' && linItemRecordSet[i]["Product Type Code"] != "Promotion")
              {
                $("#s_" + appletseqorderentry + "_l tr#" + (i + 1)).trigger("click");
