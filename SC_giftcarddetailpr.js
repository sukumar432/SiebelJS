...
function validateInputGC(thisPM) {
    ...
    if (thisPM.Get("SCStoreUser") == "Y" && giftCardFlag != "Y") {
        ...
        retVal = false;
    } else if (thisPM.Get("SCChatUser") != "Y" && thisPM.Get("SCVoicerUser") != "Y" && giftCardFlag != "Y") {
    ...
    } else if ((thisPM.Get("SCChatUser") == "Y" || thisPM.Get("SCVoicerUser") == "Y") && giftCardFlag == "Y" && thisPM.Get("sGetGCBalanceFlag") =="Y") {
        if (!thisPM.Get("sGiftCardNumber")) {
            errorCount++;
            errorMessage += errorCount + ". Gift Card#\n"
            retVal = false;
        }
        if (!thisPM.Get("sGiftPin")) {
            errorCount++;
            errorMessage += errorCount + ". PIN #\n"
            retVal = false;
        }
        if ((thisPM.Get("sGiftCardNumber") != "") && thisPM.Get("sGiftCardNumber").length != 16) {

            errorMessage1 += "Please Enter Exact 16 digit Gift card number"
            retVal = false;
        }
        if ((thisPM.Get("sGiftPin") != "") && thisPM.Get("sGiftPin").length != 4) {

            errorMessage2 += "Please Enter Exact 4 digit PIN"
            retVal = false;
	 ...
	 else if ((thisPM.Get("SCChatUser") == "Y" || thisPM.Get("SCVoicerUser") == "Y") && giftCardFlag == "Y" && thisPM.Get("GetGCBalanceFlag") =="N") {
...
if (typeof (SiebelAppFacade.GiftCardDetailPR) === "undefined") {
    ...
    define("siebel/custom/SC_giftcarddetailpr", ["siebel/phyrenderer", "siebel/custom/SelectComfort/bootstrap.min.js", "siebel/custom/SelectComfort/SC_OUI_Methods"],
        function () {
            SiebelAppFacade.GiftCardDetailPR = (function () {
                    ...
                    GiftCardDetailPR.prototype.Init = function () {
                        ...
                        giftCardFlag = SiebelApp.S_App.GetProfileAttr("GiftCardFlag");
                        poleDisplayFlg = SiebelApp.S_App.GetProfileAttr('PoleDisplayOUI');
                        fitFlag = SiebelApp.S_App.GetProfileAttr('FeatureFlag');
                        //sChatUser = "Y";
                        //var ccPM = this.GetPM();
                    ...
                    GiftCardDetailPR.prototype.ShowUI = function () {
                        ...
                        var PM = this.GetPM();
                        if (giftCardFlag == "Y" && PM.Get("SCStoreUser") == "Y" && fitFlag == "Y") {
                        ...
                        if (giftCardFlag == "Y") {
                            $("span [id ^= 'SC_Payment_Link_Label']").hide();
                            $("span [id ^= 'SC_Semaphone_Capture_Status_Label']").hide();
                    ...
                    GiftCardDetailPR.prototype.BindEvents = function (controlSet) {
                        ...
                        $("#s_" + this.GetPM().Get("GetFullId") + "_div").parent().delegate("#" + giftcardbalance.GetInputName() + "_Ctrl",
                            ...
                            function (evt) {
								...
								if(giftCardFlag =="Y")
								{
									PM.SetProperty("sGetGCBalanceFlag", "Y");
								}
                                ...
                                if (validateInputGC(PM)) {
                                    ...
                                    var GiftCardNum = $("[name=" + evt.data.ctrls["SC Credit Card Number"].GetInputName() + "]").val();
                                    var GiftPIN =  $("[name=" + evt.data.ctrls["VerificationNumber"].GetInputName() + "]").val();                                   
                                    if (giftCardFlag == "Y" && PM.Get("SCStoreUser") == "Y" && GiftCardNum == "" && GiftPIN == "" && fitFlag == "Y") {
                                        if (poleDisplayFlg == "N") {
                                            ...
                                            if (termId == "") {
                                                ...
                                                alert('Please ask the customer to swipe their Sleep number Gift Card.');
                        ...
                        //Start: LIKHITHA: added below code for terminal US
                        ...
                        $(document).on('click', '#SC-save-terminal-GC', function (event) {
                            ...
                            if (terminalSelected == "Select Terminal") {
                                ...
                                setTimeout(() => {
                                    ...
                                    SiebelApp.S_App.GetActiveView().GetAppletMap()['SC Payment Detail - Gift Card'].GetPModel().OnControlEvent("GETGCBALANCE_BUTTON_CLICK");
                        ...
                        $("#s_" + this.GetPM().Get("GetFullId") + "_div").parent().delegate("#" + authorizeButton.GetInputName() + "_Ctrl",
                            ...
                            function (evt) {
                                ...
                                PM.SetProperty("sGiftCardNumber", paymentBC.GetFieldValue("SC Credit Card Number"));
                                PM.SetProperty("sGiftPin", paymentBC.GetFieldValue("SC CVV Number"));
								if(giftCardFlag =="Y" && (sChatUser == "Y" || sVoiceUser == "Y"))
								{
									PM.SetProperty("sGetGCBalanceFlag", "N");
								}
                            ...
                            if (sAccountNumber.length == 0 && paymentBC.GetFieldValue("SC Payment Status") == "Entered") {
                                ...
                                if (write) {
                                    ...
                                    if (sPage.length > 0) {
                                        ...
                                        $("#SC-SRO-manual-gc-voice-payment").modal({
                                            backdrop: 'static'
                                        });
                                        $.ajax({
                                            type: 'GET',
