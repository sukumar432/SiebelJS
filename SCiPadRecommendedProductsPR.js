...
if (typeof (SiebelAppFacade.SCOrderEntryLineItemListAppletPR) === "undefined") {
    ...
    define("siebel/custom/SelectComfort/SCiPadRecommendedProductsPR", ["siebel/jqgridrenderer", "siebel/custom/SelectComfort/SC_OUI_Methods", "siebel/custom/SelectComfort/SC_OUI_Markups"],
        function () {
            SiebelAppFacade.SCOrderEntryLineItemListAppletPR = (function () {
                var sorderPM = "",
                    sorderBC = "",
                    FieldQueryPair, prRecomrcdset, TotalLineItemPrice, scProfileAttr, profileAttrs, PriceList;
                var sP2PEFlag = SiebelApp.S_App.GetProfileAttr("P2PEFlag");
                //LikhithaK:Start:SFSTRY0003472
                var strIpadFeatureFlg = SiebelApp.S_App.GetProfileAttr("IpadFeatureFlag");
                var strTerminalId = SiebelApp.S_App.GetProfileAttr('TerminalSelected');
                ...
                SCOrderEntryLineItemListAppletPR.prototype.ShowUI = function () {
                    ...
                    var a = "",
                        slectedlen, insertlength, skuNumberId, skurecNumber, skauquanId, skuquantity, Rejclick = "N",
                        prosedRecLen = 0;
                    ...
                    var SearchSpecification, ChildCount, Child, BSData, FieldsArray, prodid = [],
                        skuBSData;
                    ...
                    if (parentskunumber.length != 0) {
                        ...
                        FieldQueryPair = {
                            "Catalog Id": CatalogIDs,
                            "Category Status": "Y",
                            "Price List Id": PriceList,
                        ...
                        var SeqOrderKeys = ["TOTAL PROTECTION MATTRESS PAD", "REMOTE", "DUAL TEMP", "DISPOSAL", "UPHOLSTERED FURNITURE"],
                            position, tempseqKeys = [];
                        ...
                        function setProductPAs(name, value) {
                            return new Promise(function (resolve, reject) {
                                ...
                                inPS.SetProperty("Name", name);
                                inPS.SetProperty("Value", value);
                                ...
                                ai.opdecode = true;
                                ai.errcb = function () {
                                    SiebelJS.Log("Error while setting the product profile attr");
                                    reject("Failed at the time of Adding the " + name + " Profile Atribute");
                                };
                                ai.cb = function () {
                                    resolve("Added the " + name + " Profile Atribute Successfully");
                                }
                                Bservice.InvokeMethod("SetProfileAttr", inPS, ai);
                            });
                        }

                        function addSelectedProduct(selProductDetails) {
                            //return new Promise(function(resolve, reject) {
                            var SKU = selProductDetails[prosedRecLen].SKU;
                            var Quantity = selProductDetails[prosedRecLen].QTY;
                            var prodClass = selProductDetails[prosedRecLen].PCLASS;
                            var lastItem = selProductDetails[prosedRecLen].LSTITEM;
                            ...
                            ai.errcb = function () {
                                ...
                                reject("Failed at the time of Adding the Product");
                            };
                            ai.cb = function () {
                                SiebelJS.Log('Added sku# to the order');
                                if (prodClass == "REMOTE") {
                                    ...
                                    ai1.cb = function () {
                                        SiebelJS.Log('Added Product Successfully');
                                        if (lastItem) {
                                            var Bservice = '',
                                                inPS = '',
                                                outPS = '';
                                            ...
                                            addSelectedProduct(selProductDetails);
                                        }
                                    }
                                    linItemPM.ExecuteMethod("InvokeMethod", "WriteRecord", inPropSet, ai1);
                                } else {
                                    if (lastItem) {
                                        var Bservice = '',
                                            inPS = '',
                                        ...
                                        $("#custommaskoverlay").hide();
                                        SiebelApp.S_App.GotoView("SC Create Sales Order View OUI", "", "", "");
                                        //resolve("Added New Product");
                                    } else {
                                        addSelectedProduct(selProductDetails);
                                    }
                                }
                            }
                            SiebelApp.S_App.GetActiveView().GetAppletMap()["SC Order Entry Line Item List Applet"].GetPModel().ExecuteMethod("InvokeMethod", "AddLineItem", inPropSet, ai);
                            //});
                        }

                        function addProductDirectly(skurecNumber, skuquantity, prodClass, lastItem) {
                            addSelectedProduct(skurecNumber, skuquantity, prodClass, lastItem).then(
                                function (result) {
                        ...
                        $("#sc-accept-sku").click(function () {
                            ...
                            setTimeout(function () {
                                ...
                                inPS = SiebelApp.S_App.NewPropertySet();
                                outPS = SiebelApp.S_App.NewPropertySet();
                                var slectedlen = $('.sc-rec-checkbox').filter(function () {
                                    return $(this).val();
                                }).length;
                                insertlength = 0;
                                ...
                                $('.sc-rec-checkbox').each(function () {
                                    if ($(this).val() != "") {
                                        ...
                                        //LikhithaK:Start:SFSTRY0003472
                                        if (strIpadFeatureFlg == "Y") {
                                            var skudescid = $(this).attr('id') + "desc";
                                            var skudesctext = $("#" + skudescid).text();
                                            skudesctext = skudesctext.split("-");
                                            skudesctext = skudesctext[1];
                                            var skupriceId = $(this).attr('id') + "price";
                                            var skuprice = $("#" + skupriceId).text();
                                            skuprice = skuprice.replace(/[$,]/g, "");
                                            sLineDetails.push(skudesctext);
                                            sLineDetails.push("QTY: " + skuquantity + "	PRICE: $" + parseFloat(skuprice).toFixed(2));
                                ...
                                console.log(selProductDetails);
                                if (selProductDetails.length > 0) {
                                    addSelectedProduct(selProductDetails);
                                }
                                ...
                                $('.sc-rec-checkbox').each(function(){
                                    if($(this).val()!=""){
                                        ...
                                        promises.push(addSelectedProduct(skurecNumber, skuquantity, prodClass, lastItem));
                                    }
                                });
                                ...
                                if (strIpadFeatureFlg == "Y") {
                                    if (localStorage.getItem("InvokepoleDisplay") == "Y" && sP2PEFlag == "Y") {
                                        ...
                                        if (insertlength == slectedlen) {
                                            ...
                                            //Explicitly storing profile attrib in local storage since GetProfileAttr was returning undefined in SC_OUI_Methods.P2PEDispalyMultiLineItems when called from here
                                            profileAttrJSON = JSON.parse(localStorage.getItem("ProfileAttr"));
                                            profileAttrJSON['FeatureFlag'] = strIpadFeatureFlg;
                                            profileAttrJSON['TerminalSelected'] = strTerminalId;
                                            localStorage.setItem('ProfileAttr', JSON.stringify(profileAttrJSON));
                ...
                SCOrderEntryLineItemListAppletPR.prototype.BindEvents = function () {
                    ...
                    var addonsmarkup = "",
                        descId, skudesc, skuPrice, priceId, TotalPrice = 0.00,
                        qvalue;
