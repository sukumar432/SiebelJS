    ...
    function ()
    {
      SiebelApp.SC_OUI_Methods = (function ()
      {
          ...
          if (isTerminalPresent == "none")
          {
            ...
            if (childNum > 0)
            {
              ...
              while (terminalCount > 0)
              {
                ...
                {
                  optionsString += '<option class="SC-SO-Option"  term-id="' + termId + '" disabled="disabled">' + termName + ' - ' + termId + '</option>';
                }
            ...
            options.each(function (i, o)
            {
              ...
              $(o).attr('term-id', termString.substr((termString.indexOf(' - ') + 3), termString.length));
        ...
        SC_OUI_Methods.prototype.ExecuteListAppletFrame = function (l, FieldQueryPairSearch, appletName)
        {
          ...
          {
            $.each(FieldQueryPairSearch, function (inx, vlv)
            { //again iterate over FieldQueryPairSearch
        ...
        {
          strFeatureFlg = SiebelApp.S_App.GetProfileAttr('FeatureFlag'); //SL;9July24;Added for line item display
              ...
              if (index === LineDetails.length - 1) //if last line item then call the BS to build the OutputXHTML
              {
                ...
                if (objBS)
                {
                  ...
                  if (strTerminalId === "" || strTerminalId === null)
                  {
                    ...
                    outputPS = objBS.InvokeMethod("GetTerminalId", inPS);
                    strTerminalId = outputPS.GetChild(0).GetProperty('TerminalId');
                  }

                  outputPS = objBS.InvokeMethod("ItemDisplay", sDMultiInputs);
                  strOutputXHTML = outputPS.GetChild(0).GetValue();
