	...
	define("siebel/custom/SC_giftcarddetailpm", [], function() {
		SiebelAppFacade.GiftCardDetailPM = ( function(){
			...
			GiftCardDetailPM.prototype.Init = function(){
				...
				this.AddProperty( "sGetGCBalanceFlag", "N" );
					...
					function() {
						...
						else if(window.PM.Get("SCChatUser") == "Y" || window.PM.Get("SCVoicerUser") == "Y" ||  giftCardFlag =="Y")
						{
							...
							if( service ){
								...
								var aGiftCardNumber=  this.Get("sGiftCardNumber");
								var aGiftPin=  this.Get("sGiftPin");
								outPS=service.InvokeMethod("GetGiftCardBalance", inPS);
								var balance = outPS.GetChild(0).GetChild(0).GetProperty("ErrMsg");
								balance =balance.substring(balance.length -37).split(" ")[0];
								if( giftCardFlag =="Y"){
								if(balance !="" && balance !="e" )
								{
									this.SetProperty("GetGCBalanceFlag", "Y");
										...
										sinPS.SetProperty('Name','pGiftCardNumber');
										sinPS.SetProperty('Value', aGiftCardNumber);
									if(sBservice)
											{
												soutPS = sBservice.InvokeMethod("SetProfileAttr", sinPS);
											}
										sinPS.Reset();	
										sinPS.SetProperty('Name','pGiftPin');
										sinPS.SetProperty('Value',aGiftPin);
										if(sBservice)
											{
												soutPS = sBservice.InvokeMethod("SetProfileAttr", sinPS);
											}
									...
									this.SetProperty("GetGCBalanceFlag", "N");
								}
								}
							}
						}
