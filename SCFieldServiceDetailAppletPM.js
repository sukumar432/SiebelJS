if (typeof(SiebelAppFacade.SCFieldServiceDetailAppletPM) === "undefined") {

 SiebelJS.Namespace("SiebelAppFacade.SCFieldServiceDetailAppletPM");
 define("siebel/custom/SelectComfort/SCFieldServiceDetailAppletPM", ["siebel/pmodel"],
  function () {
   SiebelAppFacade.SCFieldServiceDetailAppletPM = (function () {
	   var siebConsts = SiebelJS.Dependency( "SiebelApp.Constants" );

    function SCFieldServiceDetailAppletPM(pm) {
     SiebelAppFacade.SCFieldServiceDetailAppletPM.superclass.constructor.apply(this, arguments);
    }

    SiebelJS.Extend(SCFieldServiceDetailAppletPM, SiebelAppFacade.PresentationModel);

    SCFieldServiceDetailAppletPM.prototype.Init = function () {
     SiebelAppFacade.SCFieldServiceDetailAppletPM.superclass.Init.apply(this, arguments);
		...
		this.AttachEventHandler(siebConsts.get("PHYEVENT_INVOKE_CONTROL"),
			function (methodName, PsInputArg,ai,returnStructure) {
				if (methodName == "SCFSBookNow"){
					...
					var hdsrBC = SiebelApp.S_App.GetActiveView().GetAppletMap()[thisPM.GetObjName()].GetBusComp();
    ...
    SCFieldServiceDetailAppletPM.prototype.Setup = function (propSet) {
    SiebelAppFacade.SCFieldServiceDetailAppletPM.superclass.Setup.apply(this, arguments);
    }
		function PreInvokeMethod(methodName,PsInputArg,ip,returnStructure){
			try{
					
				if(methodName=="PostChanges" && PsInputArg.propArrayLen==1){
					if(PsInputArg.propArray.hasOwnProperty("SWEPOC"))
						if(PsInputArg.GetProperty("SWEPOC")=="SC Product Location" || PsInputArg.GetProperty("SWEPOC")=="Priority"){
							if(this.ExecuteMethod("CanInvokeMethod", "WriteRecord")){
								this.ExecuteMethod("InvokeMethod", "WriteRecord", null, false);
							}
							if(this.ExecuteMethod("CanInvokeMethod", "ExecuteQuery")){
								this.ExecuteMethod("InvokeMethod", "ExecuteQuery", null, false);
							}
					}
				}
			}
    ...
    return SCFieldServiceDetailAppletPM;
   }()
  );
  return "SiebelAppFacade.SCFieldServiceDetailAppletPM";
 })
}