    ...
    function ()
    {
      SiebelAppFacade.SCSalesOrderCreateFormAppletPR = (function ()
      {
        ...
        var pm = "",
          ...
          markup = "",
          headermarkup = "",
          storechangemarkup, Lov, AccountOrder = false,
          a = "",
          b = "",
          billAddList = "";
        var selectstoreid = "",
          c = "",
          StoreLocation, LoginId, storeNumber, BService, billToContactRecordSet, responsibilityadmin, humancapitalresp, taxExeChange = 'N',
          Reason = "";
        ...
        SCSalesOrderCreateFormAppletPR.prototype.ShowUI = function ()
        {
          ...
          setTimeout(function ()
          {
            ...
            var isTerminalPresent = $('.profile-block #SC-terminal-tile').css('display');
            var terminalValue = SiebelApp.S_App.GetProfileAttr('TerminalSelectedName');
            if (isTerminalPresent == "none" && terminalValue != "" && fitFlag == "Y")
            {
              $('.profile-block #SC-terminal-tile span').text(terminalValue);
              $('.profile-block #SC-terminal-tile').css('display', '');
          ...
          $('#SC-terminal-offline-popup').parent('div').parent('#applet1').remove();
          ...
          if (RecordSet[0]["SC Sub-Type"] == "Wholesale" || RecordSet[0]["SC Sub-Type"] == "QVC" || RecordSet[0]["SC Sub-Type"] == "Commercial" || RecordSet[0]["SC Sub-Type"] == "Internal" || RecordSet[0]["SC Sub-Type"] == "HSN")
          {
            ...
            AccountOrder = true;
            localStorage.setItem("sAccountOrder", "Y"); //LikhithaK:03Mar2025:US#SFSTRY0003418:Added as part of Billing Address Dropdown
          ...
          {
            AccountOrder = false;
            localStorage.setItem("sAccountOrder", "N"); //LikhithaK:03Mar2025:US#SFSTRY0003418:Added as part of Billing Address Dropdown
          }
          //NGOLLA Defect #710
          if (AccountOrder)
          {
          ...
          //LikhithaK:21Feb2025:US#SFSTRY0003418:Added as part of Billing Address Dropdown
          if (AccountOrder != true)
          {
            var BillingCreateNewAddress = SCSalesCreateMarkup.CreateNewBillingAddress();
            $("#SC-add-bill-address").html(BillingCreateNewAddress);
          }
          //LikhithaK:21Feb2025:US#SFSTRY0003418:Added as part of Billing Address Dropdown
          ...
          //LikhithaK:21Feb2025:Start:US#SFSTRY0003418:Added as part of Billing Address Dropdown
          $(document).on('change', "#SC-SO-Bill-NA-Address", function (event)
          {
            event.preventDefault();
            if ($("#SC-SO-Bill-NA-Apt").val() == "")
              $("#SC-SO-Bill-NA-Apt").focus();
          });
          $(document).on('focus', "#SC-SO-Bill-NA-Apt", function (event)
          {
            event.preventDefault();
            if ($("#SC-SO-Bill-NA-Apt").val() == "")
              $("#Bill-SNAPToolTip").show();
          });
          $(document).on('focusout', "#SC-SO-Bill-NA-Apt", function (event)
          {
            event.preventDefault();
            $("#Bill-SNAPToolTip").hide();
          });
          $(document).on('click', "#SC-SO-Bill-SaveNewAddress", function ()
          {
            SiebelJS.Log("Saving New Billing Address");
            var errorCodes = SCErrorCodes.errorcodes();
            $("#SC-SO-Bill-CreateNewAddress").validate(
            {
              rules:
              {
                address:
              ...
              {
                event.preventDefault();
                var BillToContactRecordSet = SiebelApp.S_App.GetActiveView().GetAppletMap()["SC Sales Order Entry Form Applet OUI"].GetBusComp().GetRecordSet();
                var bConId = BillToContactRecordSet[0]["Bill To Contact Id"];
                SiebelJS.Log("Bill to Contact Id is -- In Address creation:" + bConId);
                ...
                setTimeout(function ()
                {
                  ...
                  inPS.SetProperty("ContactId", bConId);
                  inPS.SetProperty("SC StreetAddr1", $("#SC-SO-Bill-NA-Address").val());
                  inPS.SetProperty("SC StreetAddr2", $("#SC-SO-Bill-NA-Apt").val());
                  inPS.SetProperty("SC City", $("#SC-SO-Bill-NA-City").val());
                  inPS.SetProperty("SC State", $("#SC-SO-Bill-NA-State").val());
                  inPS.SetProperty("SC Country", "");
                  inPS.SetProperty("SC PostalCode", $("#SC-SO-Bill-NA-ZipCode").val());
                  ...
                  var bAddrid = outPS.GetChild(0).GetProperty("AddressId");
                  ...
                  document.getElementById("SC-SO-Bill-CreateNewAddress").reset();
                  $("#SC-SO-Bill-NA-Address").removeClass("is-active");
                  $("#SC-SO-Bill-NA-Apt").removeClass("is-active");
                  $("#SC-SO-Bill-NA-City").removeClass("is-active");
                  $("#SC-SO-Bill-NA-State").removeClass("is-active");
                  $("#SC-SO-Bill-NA-ZipCode").removeClass("is-active");
                  $("#SC-SO-Bill-NA-Address").removeClass("is-completed");
                  $("#SC-SO-Bill-NA-Apt").removeClass("is-completed");
                  $("#SC-SO-Bill-NA-City").removeClass("is-completed");
                  $("#SC-SO-Bill-NA-State").removeClass("is-completed");
                  $("#SC-SO-Bill-NA-ZipCode").removeClass("is-completed");
                }, 1000);
              } //closing submit handler

            });
          });
          //LikhithaK:21Feb2025:End:US#SFSTRY0003418:Added as part of Billing Address Dropdown
          ...
          $('#SC-terminal-tile').click(function ()
          {
            ...
            $('.SC-add-terminals .modal-dialog .modal-content .modal-header').find('.header-content').append('<button type="button" class="close SC-close-popup-term blue-bg" id="sc-terminal-close-id" data-dismiss="modal">×</button>');
        ...
        SCSalesOrderCreateFormAppletPR.prototype.BindData = function (bRefresh)
        {
          ...
          localStorage.setItem('OrderNum', RecordSet[0]["Order Number"]);
        ...
        SCSalesOrderCreateFormAppletPR.prototype.BindEvents = function ()
        {
          ...
          $("#savenexit").click(function ()
          {
            ...
            SC_OUI_Methods.CheckTerminalStatus(localStorage.getItem('OrderId')); //31July24;SL;Created for home page navigation on Terminal
          ...
          //VYADAVAL-13MAY2025-Start for Protection Plan by Extend
          $(document).on('change', '[id$="_sc-line-desc"]', function (evt)
          {
            var LineVal = $(this).val();
            var elemId1 = $(this).attr("id");
            elemId1 = elemId1.split("_")[0];
            var inPS = "",
              outPS = "",
              Bservice = "",
              fieldnames, fieldvalues;
            fieldvalues = LineVal;
            fieldnames = "SC Warranty Ref Line Detail"
            inPS = SiebelApp.S_App.NewPropertySet();
            outPS = SiebelApp.S_App.NewPropertySet();
            inPS.SetProperty("BO", "Order Entry (Sales)");
            inPS.SetProperty("BC", "Order Entry - Line Items");
            inPS.SetProperty("FieldName", fieldnames);
            inPS.SetProperty("FieldValue", fieldvalues);
            inPS.SetProperty("PickListFieldName", "SC Line Detail");
            inPS.SetProperty("SearchSpecification", "[Id] = '" + elemId1 + "'");
            Bservice = SiebelApp.S_App.GetService("SC Custom Query Simplified");
            outPS = Bservice.InvokeMethod("UpdatePicklistField", inPS);
          });
          //VYADAVAL-13MAY2025End for Protection Plan by Extend
          //LikhithaK:21Feb2025:Start:US#SFSTRY0003418:Added as part of Billing Address Dropdown
          //on change of bill to address
          $(document).on('change', '#addressto', function ()
          {
          ...
          $(document).on('click', '#SC-SO-edit-block', function ()
          {
            ...
            //LikhithaK:10Mar2025:Start:US#SFSTRY0003418:Added for making Billing dropdown readonly during Order revision.
            var RecordSetc = SiebelApp.S_App.GetActiveView().GetAppletMap()["SC Sales Order Entry Form Applet OUI"].GetPModel().Get("GetRecordSet");
            var sRevisionNum = RecordSetc[0]["Revision"];
            if (sRevisionNum > 1)
            {
              $("#addressto").addClass('read-mode');
              $("#addressto").css(
            ...
            //LikhithaK:10Mar2025:End:US#SFSTRY0003418:Added for making Billing dropdown readonly during Order revision.
          });
          ...
          $("#SC-SO-sales-cancel-changes").click(function ()
          {
            ...
            if (markup == '')
              markup += '<option class="SC-SO-Option"></option>';
            //LikhithaK:21Feb2025:US#SFSTRY0003418:Below line is added as part of Contact Billing Address Dropdown
            //Add the "ADD NEW ADDRESS" option at the end of the dropdown
            if (AccountOrder != true)
            {
