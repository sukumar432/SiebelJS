...
if (typeof(SiebelApp.SC_OUI_Methods) === "undefined") {
    ...
    define("siebel/custom/SelectComfort/SC_OUI_Methods", ["siebel/jqgridrenderer", "siebel/custom/SelectComfort/SC_OUI_Markups"],
        function() {
            SiebelApp.SC_OUI_Methods = (function() {
                var subTotal = 0.00,
                    ...
                    scLovValues = [],
                    scProfileAttr = [];
				var sSource ="";//PJAYASHA;4Nov24;Added for display summary page display
                var strFeatureFlg = "N"; //SL;9July24;Added for line item display
                ...
                //SHARATH: added below method for payment terminal UI.
                SC_OUI_Methods.prototype.RetrieveLOVValues = function (s_lookup_type, s_lookup_name) {
		 ...
		 sinPS1.SetProperty("LookupType", s_lookup_type);
         sinPS1.SetProperty("LookupName", s_lookup_name);
		...
		return LOV_val;
                ...
                SC_OUI_Methods.prototype.SCGetCurrentTime = function (PayId) {

                    var sBservice1 = SiebelApp.S_App.GetService("SC Adyen Payment Service");
          var sinPS1 = SiebelApp.S_App.NewPropertySet();
          var soutPS1 = SiebelApp.S_App.NewPropertySet();
		 sinPS1.SetProperty("Id", PayId);
         
		 
         if (sBservice1) {
            soutPS1 = sBservice1.InvokeMethod("GetServerTime", sinPS1);
          }

                    return soutPS1;
                }
                SC_OUI_Methods.prototype.ResetTerminalProfileAttrib = function() {
                    //Blank out Terminal profile attribute.
                    ...
                    inPS.SetProperty('Name', 'TerminalSelectedName');
					...
					if(VName == "SC Create Sales Order View OUI")
					{
						...
						SC_OUI_Methods.prototype.ResetTerminalFromOrder(OrderNum);
					}
					
					if(VName == "SC Order Entry - Payment View Sales OUI")
					{
						var OrderNumber = SiebelApp.S_App.GetActiveView().GetApplet('SC Sales Order Entry Form Applet OUI').GetBusComp().GetFieldValue('Order Number');
						SC_OUI_Methods.prototype.ResetTerminalFromOrder(OrderNumber);
					}
					
                }
				
				SC_OUI_Methods.prototype.ResetTerminalFromOrder = function(OrderNum) {
                ...
                SC_OUI_Methods.prototype.SCnGetDisplayTerminals = function() {
                    var isTerminalPresent = $('#SC-terminal-tile').css('display');

                    if (isTerminalPresent == "none") {
						var ActiveScreenNm = $('li[aria-controls="s_sctrl_tabScreen_noop"]').filter('.ui-tabs-active').find('a').text();

						if(ActiveScreenNm == 'Service Orders')
						 {
							 $('#_sweappmenu').on('hover',function(){$(document).trigger('click');});
						 }
						else{
							$('.main-header-container').on('hover',function(){$(document).trigger('click');});
						}
						
                        //Reset terminal list.
                        $('#terminal-list option').remove();
                        $('#terminal-list').append('<option class="SC-SO-Option" selected="selected">Select Terminal</option>');
						if(localStorage.getItem('PoleDisplayConfBypass')=="Y")
						{
							$('.SC-add-terminals').remove();
							//Show popup.
							$("#SC-add-terminal-tile-bypass").modal({
								backdrop: 'static',
								keyboard: false
							});
							$("#SC-add-terminal-tile-bypass").css({
								"display": "flex",
								"justify-content": "center",
								"align-items": "center"
							});
						}else{
							$('.SC-add-terminals-bypass').remove();
							//Show popup.
							$("#SC-add-terminal-tile").modal({
								backdrop: 'static',
								keyboard: false
							});
							$("#SC-add-terminal-tile").css({
								"display": "flex",
								"justify-content": "center",
								"align-items": "center"
							});
                        ...
                        //Retrieve terminal values and populate dropdown.
                        var tinPS = "",
                            toutPS = "",
                            tBservice = "";
                        tinPS = SiebelApp.S_App.NewPropertySet();
                        toutPS = SiebelApp.S_App.NewPropertySet();
                        tBservice = SiebelApp.S_App.GetService("Workflow Process Manager");
                        var StoreNum = SiebelApp.S_App.GetProfileAttr('SC Store Number');
                        tinPS.SetProperty('ProcessName', 'SC Get Payment Terminals SNIP API WF');
                        tinPS.SetProperty('Store Id', StoreNum);
                        var terminalSet = "";
                        var ActiveTerminals = 0;
                        var optionsString = "";
						var InactiveTerminals = "";
                        if (tBservice)
                            toutPS = tBservice.InvokeMethod("RunProcess", tinPS);

                        var childNum = toutPS.GetChild(0).GetChild(0).GetChild(0).GetChildCount();
                        if (childNum > 0) {
                            terminalSet = toutPS.GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0).GetChild(0);

                            var terminalCount = terminalSet.GetChildCount();
                            ...
                            while (terminalCount > 0) {
                                var termName = terminalSet.GetChild(terminalCounter).GetProperty('TerminalName');
                                var termId = terminalSet.GetChild(terminalCounter).GetProperty('TerminalId');
                                var termStatus = terminalSet.GetChild(terminalCounter).GetProperty('Active');
                                if (termStatus == "true") {
									
								vinPS = SiebelApp.S_App.NewPropertySet();
								voutPS = SiebelApp.S_App.NewPropertySet();
								vBservice = SiebelApp.S_App.GetService("SC Adyen Payment Service");
								vinPS.SetProperty('TerminalId', termId);
								var termStat = "";
								if (vBservice)
								{
									voutPS = vBservice.InvokeMethod("GetTerminalOrderCount", vinPS);
									termStat = voutPS.GetChild(0).GetProperty('TerminalActiveFlg');
								}
									if(termStat == "N")
									{
										optionsString += '<option class="SC-SO-Option"  term-id="' + termId + '">' + termName + ' - ' + termId + '</option>';
										InactiveTerminals+=","+termId+",";
									}
									if(termStat == "Y")
									{
										optionsString += '<option class="SC-SO-Option" term-id="' + termId + '">' + termName + ' - ' + termId + '</option>';
                        ...
                        if (optionsString != "")
						{
							if(localStorage.getItem('PoleDisplayConfBypass')=="Y")
							{
								$('#terminal-list-bypass').append(optionsString);
							}else{
								$('#terminal-list').append(optionsString);
							}
						}
                        if (ActiveTerminals == 0 || optionsString == "")
						{
							if(localStorage.getItem('PoleDisplayConfBypass')=="Y")
							{
								$('#terminal-list-bypass').append('<option class="SC-SO-Option" term-id="No Terminal Available">No Terminal Available</option>');
							}else{
								$('#terminal-list').append('<option class="SC-SO-Option" term-id="No Terminal Available">No Terminal Available</option>');
							}
						}
						...
						if(localStorage.getItem('PoleDisplayConfBypass')=="Y")
							{
								 options = $('#terminal-list-bypass option').not(':first');
							}else
							{
								 options = $('#terminal-list option').not(':first');
							}
                        ...
                        options.each(function(i, o) {
                            ...
                            o.value = arr[i].v;
                            $(o).text(arr[i].t);
                            var termString = arr[i].t;
                            if (termString == "No Terminal Available") {
                                $(o).attr('term-id', 'No Terminal Available');
                                $(o).attr('term-name', 'No Terminal Available');
                            } else {
                               terminalId = termString.substr((termString.indexOf(' - ') + 3), termString.length);
                                $(o).attr('term-id', terminalId);
                                $(o).attr('term-name', termString.substr(0, termString.indexOf(' - ')));
								if(InactiveTerminals.indexOf(terminalId) != -1)
								{
                ...
                SC_OUI_Methods.prototype.PrimaryButton = function(pm, concat) {
                    ...
                    $("#Last" + appletid).click(function() {
                        ...
                        if (SiebelApp.S_App.GetActiveView().GetName() == "SC Account 360 View OUI") {
                            SCOUIMarkups.Sorting_Contacts();
                            SCOUIMarkups.Sorting_Attachments();
                            SCOUIMarkups.Sorting_Orders();
                        }
                    });
                    ...
                    (function(proxied) {
                        window.alert = function() {
                            if ((arguments[0].includes("Method GotoLastSet is not allowed here")) || (arguments[0].includes("Method GotoFirstSet is not allowed here")) || (arguments[0].includes("Method GotoNextSet is not allowed here")) || (arguments[0].includes("Method GotoPreviousSet is not allowed here"))) {} else
                ...
                SC_OUI_Methods.prototype.verifyUserLocation = function(disaflag, posRegArray) {
					...
					var ScOrderChannel = SiebelApp.S_App.GetActiveView().GetAppletMap()["SC Sales Order Entry Form Applet OUI"].GetBusComp().GetFieldValue("SC Location Type");
					var ScPrimaryDivType = SiebelApp.S_App.GetProfileAttr("SC Primary Division Type");
					var ScPrimaryDivSubType = SiebelApp.S_App.GetProfileAttr("SC Primary Division Sub Type");
                    ...
                    //start:code for if disa is not installed verfying user location with order sale location
                        ...
                        if (salelocationnumber != mystoreNumber && storeuser == "Y" && UserStoreNumber != "") {
							if ((ScOrderChannel == "ECOM" && ScOrdersubchannel == "WEBSITE") || (ScPrimaryDivType == "DIRECT" && ScPrimaryDivSubType == "CS"))
							{
							 return 2;
							 }
							 else{
                ...
                SC_OUI_Methods.prototype.ClearP2PEPoleDisplay = function(orderId) {
					...
					strFeatureFlg = SiebelApp.S_App.GetProfileAttr('FeatureFlag'); //PJAYASHA:Added for payment transformation to bypass Clear display WF call 
                ...
                SC_OUI_Methods.prototype.TaxDetailsP2PEDisplay = function(orderId, productsJSON, FreightJSON) {
                    var LineDetails = [];
					strFeatureFlg = SiebelApp.S_App.GetProfileAttr('FeatureFlag'); //PJAYASHA:Added for payment transformation to bypass display summary page 
					if (strFeatureFlg == "Y"){
						sSource ="Order Summary";
					 }
                    if (strFeatureFlg != "Y"){	
                ...
                //YashodeepJ
                SC_OUI_Methods.prototype.PrintInvoice = function(bip_id, bip_ordernumber) {
                    ...
                    setTimeout(function() {
                        if (bip_id != null) {
                            ...
                            OutPS3 = Service3.InvokeMethod("SC Commercial Invoice Generation", InPS3);
				...
				SC_OUI_Methods.prototype.PrintReceipt = function(bip_id, bip_ordernumber) {
                    SiebelJS.Log("on click of invoice generation");
                    SiebelJS.Log("bip id:" + bip_id);
                    SiebelJS.Log("bip ordernumber:" + bip_ordernumber);
                    $("body").trigger('Custom.Start');
                    setTimeout(function() {
                ...
                //31July24;SL;Created for home page navigation on Terminal
                SC_OUI_Methods.prototype.CheckTerminalStatus = function(orderId) {
                    strFeatureFlg = SiebelApp.S_App.GetProfileAttr('FeatureFlag'); //SL;14Aug2424;Added for payment transformation
                    if (strFeatureFlg === "Y") {
                        var objBS, psInp, psOut;
                        if (orderId !== "" && orderId !== null) {
                            objBS = SiebelApp.S_App.GetService("Workflow Process Manager");
                            psInp = SiebelApp.S_App.NewPropertySet();
                            psOut = SiebelApp.S_App.NewPropertySet();
                            psInp.SetProperty("Object Id", orderId);
                            psInp.SetProperty("ProcessName", "SC Check Terminal Status SNIP API WF");
                            psOut = objBS.InvokeMethod("RunProcess", psInp);
                        } else {
                            SiebelJS.Log("CheckTerminalStatus - No Order Id");
                        }
                    }
                }
                ...
                function DisplayMultiLine(orderId, LineDetails) {
                    strFeatureFlg = (typeof(SiebelApp.S_App.GetProfileAttr('FeatureFlag')) !== "undefined") ? SiebelApp.S_App.GetProfileAttr('FeatureFlag') : JSON.parse(localStorage.getItem("ProfileAttr"))['FeatureFlag']; //SL;9July24;Added for line item display
                    ...
                    for (var index = 0; index < LineDetails.length; index++) {
                        ...
                        if (strFeatureFlg !== "Y") //SL;9July24;Added to bypass Shift4 API call
                        {
                        ...
                        } else //SL;9July24;Call Adyen API
                        {
                            if (index === LineDetails.length - 1) //if last line item then call the BS to build the OutputXHTML
                            {
                                var strOrderNum, strTerminalId;
                                var appletMap = SiebelApp.S_App.GetActiveView().GetAppletMap()
                                for (var applet in appletMap) {
                                    if (applet === "SC Sales Order Entry Form Applet OUI" || applet === "Order Entry - Order Form Applet") {
                                        strOrderNum = appletMap[applet].GetBusComp().GetFieldValue("Order Number")
                                        break;
                                    }
                                }

                                var inPS = SiebelApp.S_App.NewPropertySet();
                                var outputPS = SiebelApp.S_App.NewPropertySet();
                                var outPS = SiebelApp.S_App.NewPropertySet();
                                var strOutputXHTML;
                                var objBS = SiebelApp.S_App.GetService("SC Adyen Payment Service");
                                if (objBS) {
                                    strTerminalId = SiebelApp.S_App.GetProfileAttr('TerminalSelected');
                                    strTerminalId = (typeof(strTerminalId) !== "undefined") ? strTerminalId : JSON.parse(localStorage.getItem("ProfileAttr"))['TerminalSelected'];
                                    if (strTerminalId === "" || strTerminalId === null) {
                                        inPS.SetProperty('Order Number', strOrderNum);
                                        outPS = objBS.InvokeMethod("GetTerminalId", inPS);
                                        strTerminalId = outPS.GetChild(0).GetProperty('TerminalId');
                                    }
									sDMultiInputs.SetProperty("Source", sSource);//PJAYASHA;4Nov24;Added for display summary page display
                                    outputPS = objBS.InvokeMethod("ItemDisplay", sDMultiInputs);
                                    strOutputXHTML = (typeof(outputPS) !== "undefined") ? outputPS.GetChild(0).GetValue() : ""; //Added to handle the undefined error when invoked from RecommendedProductsPR.js
                                    sDMultiInputs.SetProperty("OutputXHTML", strOutputXHTML);
                                    sDMultiInputs.SetProperty("Terminal Id", strTerminalId);
									
                                    sDMultiInputs.SetProperty("OrderNumber", strOrderNum);
                                    sDMultiInputs.SetProperty("ProcessName", "SC Line Item Pole Display SNIP API WF"); //Override the earlier set ProcessName to call Adyen API
                                    sDMultiOutputs = sDMultiService.InvokeMethod("RunProcess", sDMultiInputs);
                                }
                            }
                        }
