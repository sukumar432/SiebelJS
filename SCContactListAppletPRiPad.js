...
if (typeof(SiebelAppFacade.SCContactListAppletPRiPad) === "undefined") {
 SiebelJS.Namespace("SiebelAppFacade.SCContactListAppletPRiPad");
 define("siebel/custom/SelectComfort/SCContactListAppletPRiPad", ["siebel/jqgridrenderer","siebel/custom/SelectComfort/bootstrap.min","siebel/custom/SelectComfort/jquery.validate.min","siebel/custom/SelectComfort/SC_OUI_Definitions","siebel/custom/SelectComfort/SC_OUI_Methods","siebel/custom/SelectComfort/SC_OUI_Markups","siebel/custom/SelectComfort/SCContactSearchMarkup","siebel/custom/SelectComfort/SCCreateContactMarkup","siebel/custom/SelectComfort/SCCreateContactFunctionality","siebel/custom/SelectComfort/SCErrorCodes"],
  function () {
   SiebelAppFacade.SCContactListAppletPRiPad = (function () { 
       
	   var pm="", appletId="",appletSeq="",recordSet="",FieldQueryPair="",options="",i=0,ActiveView="",a=true,b=true,c=true,d=true,e=true,f=true,g="";
	   var responsibility="",currentValue="",searchCount=2,refsearchCount=2,storeNumber="",StoreLocation="",recentRecords,rescentRecordsMarkup="",refsearchfields="";
	   var LoginId="",next=1,currentValue="",csearchfields,selectedFieldId="",recentPM,recentRecId,recentRecSeq,refby="N",sc_reffid="",selected_RefConId="",RefById="",selected_RefConIndex="";
    ...
    function SCContactListAppletPRiPad(pm) {
      SiebelAppFacade.SCContactListAppletPRiPad.superclass.constructor.apply(this, arguments);
    }

    SiebelJS.Extend(SCContactListAppletPRiPad, SiebelAppFacade.JQGridRenderer);

    SCContactListAppletPRiPad.prototype.Init = function () {
     SiebelAppFacade.SCContactListAppletPRiPad.superclass.Init.apply(this, arguments);
    ...
    SCContactListAppletPRiPad.prototype.ShowUI = function () {
     SiebelAppFacade.SCContactListAppletPRiPad.superclass.ShowUI.apply(this, arguments);
	   ...
	   //hiding tool tip.
		...
		g=SiebelApp.S_App.GetActiveView().GetAppletMap()["SHM Contact LOY Member List Applet"].GetPModel().Get("GetFullId");
		$("#"+g).hide();
		g=SiebelApp.S_App.GetActiveView().GetAppletMap()["SC LOY Member Vouchers List Applet"].GetPModel().Get("GetFullId");
		$("#"+g).hide();
		$("#S_A3").hide();		
        $("#S_A1").hide();
		/*g=SiebelApp.S_App.GetActiveView().GetAppletMap()["SC LOY Member List Applet"].GetPModel().Get("GetFullId");
		$("#"+g).hide();
		g=SiebelApp.S_App.GetActiveView().GetAppletMap()["Recent Records List Applet"].GetPModel().Get("GetFullId");
		$("#"+g).hide();*/
		...
		if(SearchFields==undefined||SearchFields==null||SearchFields.length==0){
			...
			markup+='                            <div id="field">';
			markup+='                                <select class="select-box con-select-box margin-top " id="field0" name="prof1">';
			...
			markup+='                                </select>';
			markup+='                                <input type="text" tabindex="1" id="ifield0" class="search-box con-search-box margin-top" autocomplete="off">';
			markup+='                                <div id="b1" class="add-more margin-top "></div>';
			markup+='                            </div>';
			markup+='                            <div id="field">';
			markup+='                                <select class="select-box con-select-box margin-top" id="field1" name="prof1">';
			for(i=0;i<csearchfields.length;i++){
				if(i==1){
					markup+='                                    <option id="'+csearchfields[i]+'" value="'+csearchfields[i]+'" selected="selected">'+csearchfields[i]+'</option>';
				}else{
			...
			markup+='                                </select>';
			markup+='                                <input type="text" tabindex="2" id="ifield1" class="search-box con-search-box margin-top" autocomplete="off">';
		...
		$('#s_S_A1_div').css({position: 'absolute',left: '-1500px'});
		...
		$(".icon-img").click(function(e) {
			if(searchCount<=csearchfields.length-1){
				...
				var newIn = '<select class="select-box con-select-box margin-top" id="field' + next + '" name="field' + next + '">';
				for(i=0;i<csearchfields.length;i++){
					newIn+='<option value="'+csearchfields[i]+'">'+csearchfields[i]+'</option>';
				}
				
				newIn+='</select><input type="text" tabindex="'+(next+1)+'" id="ifield' + next + '" class="search-box con-search-box margin-top" autocomplete="off">';
				var newInput = $(newIn);
	...
	function SCTable(pm,recordSet)
	{
		...
		for(i=0;i<recordSet.length;i++){
			...
			sctablemarkup+= '                                    <td title="' + recordSet[i]["SC Customer Category"] + '">' + recordSet[i]["SC Customer Category"] + '</td>';
	...
	function sneakpeek(record){  // [Vamsi:11-01-2018: As per Itti's Comments, to pass record instead of recordset]
		var VouchersPM = SiebelApp.S_App.GetActiveView().GetAppletMap()["SC LOY Member Vouchers List Applet"].GetPModel().GetRenderer().GetPM();
		...
		markup+='<p class="category no-margin">'+record["SC Customer Category"]+'</p>';
		...
		if(record["SC Customer Category"]!=(''|""|"undefined")){
			markup+='                        <div class="SC-S-detail-item">';
			markup+='                            <span class="SC-S-detail-item-label">Customer Category :</span>';
			markup+='                            <span class="SC-S-detail-item-value">'+record["SC Customer Category"]+'</span>';
			markup+='                        </div>';
		}
	...
	SCContactListAppletPRiPad.prototype.BindData = function (bRefresh) {
     SiebelAppFacade.SCContactListAppletPRiPad.superclass.BindData.apply(this, arguments);
		//on click of mvg
		...
		$("#row"+(j+1)).dblclick(function() {
			...
			$('#sc-snkpk-customer360').click(function() {
				$("#contact-details").modal('hide');
				 $('#maskoverlay').show();
				setTimeout(function(){
					FieldQueryPair={"First Name":"[Id]='"+recordSet[k]["Id"]+"'"};
    ...
    SCContactListAppletPRiPad.prototype.BindEvents = function () {
     SiebelAppFacade.SCContactListAppletPRiPad.superclass.BindEvents.apply(this, arguments);
    ...
    SCContactListAppletPRiPad.prototype.EndLife = function () {
     SiebelAppFacade.SCContactListAppletPRiPad.superclass.EndLife.apply(this, arguments);
		SearchFields=null;
    ...
    return SCContactListAppletPRiPad;
   }()
  );
  return "SiebelAppFacade.SCContactListAppletPRiPad";
 })
}
