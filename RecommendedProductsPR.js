  ...
  define("siebel/custom/SelectComfort/RecommendedProductsPR", ["siebel/jqgridrenderer", "siebel/custom/SelectComfort/SC_OUI_Methods", "siebel/custom/SelectComfort/SC_OUI_Markups"],
    function ()
    {
      SiebelAppFacade.SCOrderEntryLineItemListAppletPR = (function ()
      {
        ...
        var strFeatureFlg = SiebelApp.S_App.GetProfileAttr('FeatureFlag'); //SL;19Aug24;Added for line item display
        var strTerminalId = SiebelApp.S_App.GetProfileAttr('TerminalSelected'); //SL;19Aug24;Added for line item display
          ...
          if (parentskunumber.length != 0)
          {
            ...
            $("#sc-accept-sku").click(function ()
            {
              ...
              setTimeout(function ()
              {
                ...
                var sLineDetails = []; //SL;19Aug24;Moved from each function
                var bInvokeP2PEDisplay = false; //SL;19Aug24;Added for line item display
                var inDisplayItemPS = SiebelApp.S_App.NewPropertySet(); //SL;19Aug24;Added for line item display
				let profileAttrJSON;//SL;19Aug24;Added for line item display
                insertlength = 0;
                $('.sc-rec-checkbox').each(function ()
                {
                  if ($(this).val() != "")
                  {
                    ...
                    if (localStorage.getItem("InvokepoleDisplay") == "Y" && sP2PEFlag == "Y")
                    {
                      //SL;19Aug24;Added below if-else for payment transformation
                      if (strFeatureFlg === "Y") //P2PEDisplay will be called with all the selected products-Adyen flow
                      {
                        if (insertlength === slectedlen)
                        {
                          bInvokeP2PEDisplay = true;
                          //SL;19Aug24;Explicitly storing profile attrib in local storage since GetProfileAttr was returning undefined in SC_OUI_Methods.P2PEDispalyMultiLineItems when called from here
                          profileAttrJSON = JSON.parse(localStorage.getItem("ProfileAttr"));
                          profileAttrJSON['FeatureFlag'] = strFeatureFlg;
                          profileAttrJSON['TerminalSelected'] = strTerminalId;
						  localStorage.setItem('ProfileAttr', JSON.stringify(profileAttrJSON));
                        }
                      }
                      else //P2PEDisplay will be called with one selected product at a time-Shift4 flow
                      {
                        bInvokeP2PEDisplay = true;
                        sLineDetails = [];
                      }
                      ...
                      if (bInvokeP2PEDisplay === true) //SL;19Aug24-payment transformation
